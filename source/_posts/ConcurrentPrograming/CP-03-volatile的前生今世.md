---
title: CP-03-volatile的前生今世
date: 2018-01-15 20:00:40
...
![image](https://note.youdao.com/yws/api/personal/file/5DA3E5BC73E247F38C1F07C53E12BABF?method=download&shareKey=0d33c2fc117cbae7935d1fb1da5ce4e7)
...
1. 然后线程2对变量进行自增操作，线程2也去读取变量inc的原始值，由于线程1只是对变量inc进行读取操作，而没有对变量进行修改操作，所以不会导致线程2的工作内存中缓存变量inc的缓存行无效，所以线程2会直接去主存读取inc的值，发现inc的值时10，然后进行加1操作，并把11写入工作内存，最后写入主存。
1. 然后线程1接着进行加1操作，由于已经读取了inc的值，注意此时在线程1的工作内存中inc的值仍然为10，所以线程1对inc进行加1操作后inc的值为11，然后将11写入工作内存，最后写入主存。
1. 那么两个线程分别进行了一次自增操作后，inc只增加了1。
...
## volatile底层实现
#### JVM层实现
JVM规范中定义了JMM内存模型，用来屏蔽不同硬件和操作系统的差异，以实现所有访问的一致性，其定义了每个线程有自己的工作内存和JVM的主存，分别类似OS的CPU缓存和主存。每个线程不能直接操作JVM主存，只能先放到自己的工作内存中操作，操作完成后写回主存。

JVM对加了volatile关键字的变量附加了两条语意：
1. 保证了不同线程对这个变量操作时的可见性，即一个线程对其操作时，其他线程立即可见
2. 禁止进行指令重排序


#### 操作系统层实现
通过JIT编译器生成的汇编指令来查看对volatile变量进行写操作时，加了比未加volatile的变量会多一个lock前缀的指令，该指令在多核操作系统下会引发两件事情：
1. 将当前处理器的缓存行写回到操作系统内存
2. 这个写回内存操作会使其他处理器里缓存了该内存地址的数据无效
